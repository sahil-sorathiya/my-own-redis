...
public class RpushCommand implements Command {
    ...
    public void execute(RespObject c, ClientContext ctx) throws IOException {
        ...
        //: If less than three argument passed, throw error
        //: RPUSH <key> <element1> [<element2>] ... [<elementN>]
        if(command.size() < 3) {
            ctx.respWriter.write(new RespError("ERR wrong number of arguments for 'rpush' command"));
            return;
        }
        ...
        for(int i = 2; i < command.size(); i++){
            ...
            if(isOfferAccepted == false){
                ((RespArray) val).values.add(command.get(i));
            }
        }
